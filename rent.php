...
if ($role_id == 2) {
    ...
    $stmt = $conn->prepare("SELECT rents.*, users.name AS user_name, books.title AS book_title, books.image AS book_image
                            FROM rents
                            JOIN users ON rents.user_id = users.id
                            JOIN books ON rents.book_id = books.id
                            WHERE rents.user_id = ?
                            ORDER BY rents.id ASC");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    // Admin bisa lihat semua data
    $sql = "SELECT rents.*, users.name AS user_name, books.title AS book_title, books.image AS book_image
            FROM rents
            JOIN users ON rents.user_id = users.id
            JOIN books ON rents.book_id = books.id
            ORDER BY rents.id ASC";
            ...
            <main class="flex-1 overflow-y-auto p-4 md:p-6">
                ...
                <div class="space-y-6">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <h1 class="text-2xl font-bold">List Rent</h1>
                        <div class="flex gap-2">
                            ...
                            <?php if ($role_id != 2): ?>
                            <button
                                class="inline-flex items-center justify-center text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 transition-colors duration-200">
                                <a href="http://localhost/booklibrary/dashboard/rent/form.php"
                            ...
                            <table class="w-full">
                                ...
                                <tbody class="divide-y divide-gray-200 ">
                                        ...
                                        <td class='px-6 py-4 whitespace-nowrap'>
                                            <div class='flex items-center'>
                                                <div class='text-sm font-medium'>
                                                    <?= htmlspecialchars($row["user_name"]) ?></div>
                                            </div>
                                        </td>
                                        <td class='px-6 py-4 whitespace-nowrap'>
                                            <div class='flex items-center gap-3'>
                                                <img src='../uploads/<?= htmlspecialchars($row["book_image"]) ?>'
                                                    alt='<?= htmlspecialchars($row["book_title"]) ?>'
                                                    class='h-[100px] w-[100px] object-cover rounded' />
                                                <div class='text-sm font-medium'>
                                                    <?= htmlspecialchars($row["book_title"]) ?></div>
                                            </div>
                                        </td>
                                        <td class='px-6 py-4 whitespace-nowrap text-sm'><?= $row["rent_date"] ?></td>
                                        <td class='px-6 py-4 whitespace-nowrap text-sm'><?= $row["due_date"] ?></td>
                                        <td class='px-6 py-4 whitespace-nowrap text-sm'><?= $row["return_date"] ?></td>
                                        <?php if ($role_id != 2): ?>
                                        <td
                                            class='px-6 py-4 flex flex-col gap-4 justify-center whitespace-nowrap text-center'>
                                            <a href='http://localhost/booklibrary/dashboard/rent/form.php?id=<?= $row["id"] ?>'
                                                class='inline-flex items-center justify-center text-sm font-medium rounded-md shadow-sm text-white bg-yellow-600 hover:bg-yellow-700 px-4 py-2 transition-colors duration-200'>
                                                Update
                                            </a>
                                            <a href='http://localhost/booklibrary/dashboard/rent/delete.php?id=<?= $row["id"] ?>'
                                                onclick="return confirm('Are you sure you want to delete this data?')"
                                                class='inline-flex items-center justify-center text-sm font-medium rounded-md shadow-sm text-white bg-rose-600 hover:bg-rose-700 px-4 py-2 transition-colors duration-200'>
                                                Delete
                                            </a>
                                        </td>
                                        <?php endif; ?>
                                    ...
                                    <?php
                                        ...
                                        ?>
                                    <tr>
                                        <td colspan="7" class='px-6 py-4 whitespace-nowrap text-sm text-center'>No
                                            record found.</td>
                                    </tr>
                                    <?php
                                        }
